...
{
    private $folder_question = "true_false_question";
    ...
    public function get_true_false_question_by_category(Request $request)
    {
        try {
            ...
            $data = TrueFalse_Question::inRandomOrder()->where('category_id', $request->category_id)->limit($this->config_setting['true_false_question'])->get();
    ...
    public function save_true_false_question_report(Request $request)
    {
        try {
            $validator = Validator::make($request->all(), [
            ...
            $leaderboard = new TrueFalse_Leaderborad();
            ...
            if ($this->config_setting['min_true_false_percentage'] <= $percentage) {
            ...
            if ($leaderboard->save()) {
                ...
                if ($U_score && $leaderboard->is_win_point == 1) {
                    ...
                    $U_score->increment('total_points', $this->config_setting['true_false_win_point']);

                    // Add Earn Transaction
                    $this->common->add_earn_transaction($user_id, 0, 5, $this->config_setting['true_false_win_point']);
                }
    ...
    public function get_true_false_leaderboard(Request $request) // Type = today, month, all
    {
        try {
            ...
            if ($type == "today" || $type == "month") {
                ...
                if ($type == 'today') {
                    ...
                    $data = TrueFalse_Leaderborad::select('user_id', DB::raw('SUM(score) As total_score'), DB::raw('RANK() OVER (ORDER BY total_score DESC) as rank'))
                        ->whereYear('created_at', date('Y'))->whereMonth('created_at', date('m'))->groupBy('user_id')
                        ->with('users')->orderBy('total_score', 'DESC')->orderBy('created_at', 'DESC')->limit('10')->get();
                }
                ...
                if ($type == "today") {
                    ...
                    $single_data = TrueFalse_Leaderborad::select('user_id', DB::raw('SUM(score) As total_score'), DB::raw('RANK() OVER (ORDER BY total_score DESC) as rank'))
                        ->whereDate('created_at', date('Y-m-d'))->groupBy('user_id')->with('users')
