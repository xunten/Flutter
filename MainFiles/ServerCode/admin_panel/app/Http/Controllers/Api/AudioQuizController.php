...
{
    private $folder_question = "audio_question";
    ...
    public function get_audio_question_by_category(Request $request)
    {
        try {
            ...
            $data = Audio_Question::inRandomOrder()->where('category_id', $request->category_id)->limit($this->config_setting['audio_question'])->get();
            ...
            if ($data) {
                ...
                foreach ($data as $key => $value) {
                    if ($value->audio_type == "server_video") {
                        $value['audio'] = $this->common->Get_Image($this->folder_question, $value['audio']);
                    }
                }
    ...
    public function save_audio_question_report(Request $request)
    {
        try {
            $validator = Validator::make($request->all(), [
            ...
            $leaderboard = new Audio_Leaderborad();
            ...
            if ($this->config_setting['min_audio_percentage'] <= $percentage) {
            ...
            if ($leaderboard->save()) {
                ...
                if ($U_score && $leaderboard->is_win_point == 1) {
                    ...
                    $U_score->increment('total_points', $this->config_setting['audio_win_point']);

                    // Add Earn Transaction
                    $this->common->add_earn_transaction($user_id, 0, 3, $this->config_setting['audio_win_point']);
                }
    ...
    public function get_audio_leaderboard(Request $request) // Type = today, month, all
    {
        try {
            ...
            if ($type == "today" || $type == "month") {
                ...
                if ($type == "today") {
                    ...
                    $single_data = Audio_Leaderborad::select('user_id', DB::raw('SUM(score) As total_score'), DB::raw('RANK() OVER (ORDER BY total_score DESC) as rank'))
                        ->whereDate('created_at', date('Y-m-d'))->groupBy('user_id')->with('users')
