...
@section('page_title',  __('Label.Add_Normal_Question'))
...
@section('content')
    ...
    <div class="right-content">
        ...
        <div class="body-content">
            <!-- mobile title -->
            <h1 class="page-title-sm">{{__('Label.Add_Normal_Question')}}</h1>
...
@section('pagescript')
	<script>
		function save_question(){
			...
			$.ajax({
				type:'POST',
				url:'{{ route("normalquestion.store") }}',
				data:formData,
				cache:false,
				contentType: false,
				processData: false,
				success:function(resp){
					$("#dvloader").hide();
					get_responce_message(resp, 'save_question', '{{ route("normalquestion.index") }}');
				},
