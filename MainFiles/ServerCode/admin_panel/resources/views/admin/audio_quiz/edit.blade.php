...
@section('page_title', __('Label.edit_audio_question'))
...
@section('content')
	...
	<div class="right-content">
    	...
    	<div class="body-content">
      		<!-- mobile title -->
      		<h1 class="page-title-sm">{{__('Label.edit_audio_question')}}</h1>
	    	...
	    	<div class="border-bottom row mb-3">
	    		<div class="col-sm-10">
	    			<ol class="breadcrumb">
	    				...
	    				<li class="breadcrumb-item">
	    					<a href="{{ route('audioquestion.index') }}">{{__('Label.edit_audio_question')}}</a>
	    				</li>
	    				<li class="breadcrumb-item active" aria-current="page">
	    					{{__('Label.edit_audio_question')}}
	    				</li>
	    			</ol>
	    		</div>
	    	...
	    	<div class="card custom-border-card mt-3">
	    		<form enctype="multipart/form-data" id="save_edit_question">
					...
					<input type="hidden" value="{{$result->audio}}" name="old_audio">
					<input type="hidden" value="{{$result->audio_type}}" name="old_audio_type">
					<div class="form-row">
						<div class="col-md-9">
							<div class="form-row">
								<div class="col-md-4">
							...
							<div class="form-row">
								<div class="form-group col-md-4">
									...
									<select name="audio_type" id="audio_type" class="form-control">
										<option value="server_video" {{ $result->audio_type == "server_video"  ? 'selected' : ''}}>{{__('Label.server_video')}}</option>
										<option value="external_url" {{ $result->audio_type == "external_url"  ? 'selected' : ''}}>{{__('Label.external_url')}}</option>
									</select>
								</div>
								<div class="form-group col-md-8 video_box">
									...
									<label class="mt-1 text-gray">@if($result->audio_type == 'server_video'){{{$result->audio}}}@endif</label>
								</div>
								<div class="form-group col-md-8 url_box">
									<label>{{__('Label.url')}}</label>
									<input name="audio_url" value="@if($result->audio_type != 'server_video'){{{$result->audio}}}@endif" type="url" class="form-control" placeholder="Enter URL">
								</div>
							</div>
...
@section('pagescript')
    <script>
		function save_edit_question(){
			...
			$.ajax({
				type:'POST',
				url:'{{ route("audioquestion.update", [$result->id]) }}',
				data:formData,
				cache:false,
				contentType: false,
				processData: false,
				success:function(resp){
					$("#dvloader").hide();
					get_responce_message(resp, 'save_edit_question', '{{ route("audioquestion.index") }}');
				},
		...
		var audio_type = "<?php echo $result->audio_type; ?>";
		if(audio_type == "server_video"){
			$(".url_box").hide();
		} else {
			$(".video_box").hide();
		}
