...
@section('page_title', __('Label.true_false_question'))
...
@section('content')
    ...
    <div class="right-content">
        ...
        <div class="body-content">
            <!-- mobile title -->
            <h1 class="page-title-sm">{{__('Label.true_false_question')}}</h1>

            <div class="border-bottom row mb-3">
                <div class="col-sm-10">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{__('Label.Dashboard')}}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{__('Label.true_false_question')}}</li>
                    </ol>
                </div>
                <div class="col-sm-2 d-flex align-items-center justify-content-end">
                    <a href="{{ route('truefalsequestion.create') }}" class="btn btn-default mw-150" style="margin-top:-14px">{{__('Label.add_true_false_question')}}</a>
                </div>
            </div>
                    ...
                    <form enctype="multipart/form-data" id="save_question_import">
                        ...
                        <div class="form-row pt-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <a class="btn btn-warning ml-5" href="{{ route('truefalsequestion_export') }}">{{__('Label.DOWNLOAD SAMPLE FILE HERE')}}</a>
    ...
    <script>
        $(document).ready(function() {
            var table = $('#datatable').DataTable({
                ...
                ajax: {
                    url: "{{ route('truefalsequestion.index') }}",
                    data: function(d) {
        ...
        function save_question_import() {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("truefalsequestion_import") }}',
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                success: function(resp) {
                    $("#dvloader").hide();
                    get_responce_message(resp, 'save_question_import', '{{ route("truefalsequestion.index") }}');
                },
