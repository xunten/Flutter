...
@section('page_title', __('Label.add_daily_quiz_question'))
...
@section('content')
	...
	<div class="right-content">
		...
		<div class="body-content">
			<!-- mobile title -->
			<h1 class="page-title-sm">{{__('Label.add_daily_quiz_question')}}</h1>
			...
			<div class="border-bottom row mb-3">
				<div class="col-sm-10">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item">
							<a href="{{ route('dailyquizquestion.index') }}">{{__('Label.daily_quiz_question')}}</a>
						</li>
						<li class="breadcrumb-item active" aria-current="page">
							{{__('Label.add_daily_quiz_question')}}
						</li>
					</ol>
				</div>
				<div class="col-sm-2 d-flex align-items-center justify-content-end">
					<a href="{{ route('dailyquizquestion.index') }}" class="btn btn-default mw-120" style="margin-top:-14px">{{__('Label.daily_quiz_question')}}</a>
				</div>
			</div>
			...
			<div class="card custom-border-card mt-3">
				<form enctype="multipart/form-data" id="save_question">
							...
							<div class="form-row">
								<div class="col-md-12">
									<div class="form-group">
										...
										<textarea class="form-control" rows="1" name="question" placeholder="Enter the Question" autofocus></textarea>
									</div>
								</div>
							</div>
					...
					<div class="border-top pt-3 text-right">
						...
						<a href="{{route('dailyquizquestion.index')}}" class="btn btn-cancel mw-120 ml-2">{{__('Label.CANCEL')}}</a>
...
@section('pagescript')
	<script>
		function save_question() {
			...
			$.ajax({
				type: 'POST',
				url:'{{ route("dailyquizquestion.store") }}',
				data: formData,
				cache: false,
				contentType: false,
				processData: false,
				success: function(resp) {
					$("#dvloader").hide();
					get_responce_message(resp, 'save_question', '{{ route("dailyquizquestion.index") }}');
				},
