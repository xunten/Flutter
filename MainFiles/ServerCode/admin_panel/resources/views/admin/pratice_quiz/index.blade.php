...
@section('page_title', __('Label.Pratice Question'))
...
@section('content')
    ...
    <div class="right-content">
        ...
        <div class="body-content">
            <!-- mobile title -->
            <h1 class="page-title-sm">{{__('Label.Pratice Question')}}</h1>

            <div class="border-bottom row mb-3">
                <div class="col-sm-10">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{__('Label.Dashboard')}}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{__('Label.Pratice Question')}}</li>
                    </ol>
                </div>
                <div class="col-sm-2 d-flex align-items-center justify-content-end">
                    <a href="{{ route('praticequestion.create') }}" class="btn btn-default mw-150" style="margin-top:-14px">{{__('Label.Add Pratice Question')}}</a>
                </div>
            </div>
                ...
                <div class="card-body">
                    <form enctype="multipart/form-data" id="save_pratice_question_import">
                        ...
                        <div class="form-row pt-3">
                            ...
                            <div class="col-md-6">
                                <button type="button" class="btn btn-default mw-120" onclick="save_pratice_question_import()">{{__('Label.UPLOAD CSV FILE')}}</button>
                        ...
                        <div class="sorting mr-3" style="width: 400px;">
                            ...
                            <select class="form-control" name="input_category" id="input_category">
                                ...
                                @for ($i = 0; $i < count($category); $i++)
                                    <option value="{{ $category[$i]['id'] }}" @if(isset($_GET['input_category'])){{ $_GET['input_category'] == $user[$i]['id'] ? 'selected' : ''}} @endif>
                                        {{ $category[$i]['name'] }}
                                    </option>
                                @endfor
                            </select>
                        ...
                        <div class="sorting" style="width: 300px;">
                            ...
                            <select class="form-control" name="input_level" id="input_level">
                                ...
                                @for ($i = 0; $i < count($level); $i++) 
                                    <option value="{{ $level[$i]['id'] }}" @if(isset($_GET['input_level'])){{ $_GET['input_level'] == $user[$i]['id'] ? 'selected' : ''}} @endif>
                                        {{ $level[$i]['name'] }}
                                    </option>
                                @endfor
                            </select>
    ...
    <script>
        $(document).ready(function() {
            var table = $('#datatable').DataTable({
                ...
                ajax:
                    {
                    url: "{{ route('praticequestion.index') }}",
                    data: function(d){
        ...
        function save_pratice_question_import() {
            ...
            $.ajax({
                ...
                success: function(resp) {
                    $("#dvloader").hide();
                    get_responce_message(resp, 'save_pratice_question_import', '{{ route("praticequestion.index") }}');
                },
