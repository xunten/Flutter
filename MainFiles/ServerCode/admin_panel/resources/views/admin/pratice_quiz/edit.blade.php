...
@section('page_title', __('Label.Edit Pratice Question'))
...
@section('content')
	...
	<div class="right-content">
    	...
    	<div class="body-content">
      		<!-- mobile title -->
      		<h1 class="page-title-sm">{{__('Label.Edit Pratice Question')}}</h1>			
    		...
    		<div class="border-bottom row mb-3">
    			<div class="col-sm-10">
    				<ol class="breadcrumb">
    					...
    					<li class="breadcrumb-item">
    						<a href="{{ route('praticequestion.index') }}">{{__('Label.Pratice Question')}}</a>
    					</li>
    			...
    			<div class="col-sm-2 d-flex align-items-center justify-content-end">
    				<a href="{{ route('praticequestion.index') }}" class="btn btn-default mw-120" style="margin-top:-14px">{{__('Label.Pratice Question')}}</a>
    			</div>
    		</div>
    		...
    		<div class="card custom-border-card mt-3">
    			<form enctype="multipart/form-data" id="save_edit_pratice_question">
							...
							<div class="form-row">
								<div class="col-md-4">
									<div class="form-group">
										<label>{{__('Label.QUESTION LEVEL')}}</label>
										<select class="form-control" name="question_level_master_id">
											<option value="">{{__('Label.Select Level')}}</option>
											@foreach ($classification as $key => $value)
											<option value="{{ $value->id}}" {{ $result->question_level_master_id == $value->id  ? 'selected' : ''}}> 
												{{ $value->level_name }} 
											</option>
											@endforeach  
										</select>
    				...
    				<div class="border-top pt-3 text-right">
    					...
    					<a href="{{route('praticequestion.index')}}" class="btn btn-cancel mw-120 ml-2">{{__('Label.CANCEL')}}</a>
...
@section('pagescript')
    <script>
		function save_edit_pratice_question(){
			...
			$.ajax({
				type:'POST',
				url:'{{ route("praticequestion.update", [$result->id]) }}',
				data:formData,
				cache:false,
				contentType: false,
				processData: false,
				success:function(resp){
					$("#dvloader").hide();
					get_responce_message(resp, 'save_edit_pratice_question', '{{ route("praticequestion.index") }}');
				},
